...
var mongourl = 'mongodb://user1:password1@ds149682.mlab.com:49682/mechaserpent';
...
app.post('/create', function(req, res) {
	...
	form.parse(req, function(err, fields, files) {
		...
		if (fields.building || fields.street || fields.zipcode || fields.lat || fields.lon) {
			var address = {};
			...
			if (fields.lat || fields.lon) address['coord'] = [fields.lat, fields.lon];
	...
	if (!req.session.authenticated) {
		...
		MongoClient.connect(mongourl, function(err, db) {
			...
			readRestaurant(db, criteria, {}, function(result) {
				...
				var lon = (result[0].address.coord === undefined) ? '' : result[0].address.coord[1];
				...
				res.render('view', { name: req.session.userid, result, lat, lon});
			});
		});
			...
			MongoClient.connect(mongourl, function(err, db) {
				...
				readRestaurant(db, criteria, {}, function(result) {
					...
					if (req.session.userid != result[0].owner) {
						...
						if (result[0].grades.user === undefined) result[0].grades.user = '';
						if (result[0].grades.score === undefined) result[0].grades.score = '';
						res.render('update', { name: req.session.userid, result, id, lat, lon });
					}
				});
			});
	...
	form.parse(req, function(err, fields, files) {
		...
		fs.readFile(filename, function(err, data) {
			...
			var set = {
				...
				'address.coord': [fields.lat, fields.lon],
...
app.post('/api/restaurant', function(req, res) {
	...
	if (req.body.building || req.body.street || req.body.zipcode || req.body.lat || req.body.lon) {
		var address = {};
		...
		if (req.body.lat || req.body.lon) address['coord'] = [req.body.lat, req.body.lon];
		new_r['address'] = address;
	}
